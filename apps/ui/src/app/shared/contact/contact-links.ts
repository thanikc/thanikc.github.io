export interface SocialLink {
  /** Doubles as the analytics CTA label, so it stays English in every locale. */
  ...
  /** `d` attribute of the single-path 24×24 icon. */
  readonly iconPath: string;
}

/** Ways to reach Thanik, shared by the footer and the profile hero. */
export const SOCIAL_LINKS: readonly SocialLink[] = [
  {
    label: 'Email',
    shortLabel: $localize`:Link to send Thanik an email@@contact.email.label:Email`,
    href: 'mailto:thanikc@gmail.com',
    ariaLabel: $localize`:Accessible name of the email link@@contact.email.ariaLabel:Email Thanik Cheowtirakul`,
    external: false,
    iconPath:
  ...
  },
  {
    ...
    ariaLabel: $localize`:Accessible name of the GitHub link@@contact.github.ariaLabel:GitHub repository of Thanik Cheowtirakul`,
    external: true,
    iconPath:
  ...
  },
  {
    ...
    ariaLabel: $localize`:Accessible name of the LinkedIn link@@contact.linkedin.ariaLabel:LinkedIn profile of Thanik Cheowtirakul`,
    external: true,
    iconPath:
...
/** Looks up one of the shared links by its label. */
export const contactLink = (label: string): SocialLink => {
  const link = SOCIAL_LINKS.find(candidate => candidate.label === label);
  if (!link) throw new Error(`Unknown contact link: ${label}`);
  return link;
};
