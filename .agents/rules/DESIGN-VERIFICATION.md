...
(classes, attributes, order); they cannot prove contrast, overflow, hit areas, or focus
visibility. This rule covers what they can't.
...
- Artefacts go to `tmp/design-check/` (git-ignored): `measure/*.json` per check,
  `screens/*.png` per width × route. Never commit screenshots. Delete
...
- **Gradient text** (`color: transparent` + `background-clip: text`) carries its colour in
  `background-image`, not `color` — check that property, not `color`, when verifying it.
...
The app is light-only (no theme toggle, no dark-mode tokens — removed in the to-top.ch
re-theme). Every check runs across **3 widths**, one theme, unless noted:
...
1. **Contrast.** Run axe (`AxeBuilder` with the `color-contrast` rule). Zero violations. Axe
   can't see text over the hero photo or gradients: for those, screenshot the element and
   state that contrast there was judged visually, not measured.
...
9. **Surface separation (non-text contrast).** Axe only checks *text*: it passes a page
    ...
    glance," calibrated against that failure — not a WCAG number). Cards use
    `--app-surface-raised` on `--app-surface-base` (material.scss). If this check ever
    needs loosening, look at screenshots first — don't just raise the number to make it
    pass.
10. **Section rhythm.** The gap between each section header and its content is at least
...
Take one full-page screenshot per width into `tmp/design-check/`. Metrics and axe reports
come first (Token Economy, AGENTS.md), but they are not a visual review: passing numbers
once hid cards that blended into the page and headers glued to their content. Before
reporting done, look at a crop of **every section** of each changed route (at least at
1440 px, plus 360 px for layout changes) — the hero alone is not a visual review. Name
anything seen there that no check measures, and add a check for it when it can be measured.
